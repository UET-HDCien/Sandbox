...
class Sandbox:
    ...
    def execute(self, binary_filepath, platform, sandbox_id, interpreter = None):
        ...
        pcap_command = "dumpcap -i %s -P -w %s -f 'not ((tcp dst port %d and ip dst host %s) or (tcp src port %d and ip src host %s))'"
	#pcap_command="strace ./%s"
        ...
        try: 
            ...
            qemu.expect("(qemu).*")
            ifname = "br0"#qemu.before.split("ifname=", 1)[1].split(",", 1)[0]
	    ...
	    print ssh_user
            ...
            ##
            os.system("touch " + pcap_filepath)
            os.system("chmod 777 " + pcap_filepath)
	    ...
	    #cmand=self.ssh_execute(ssh_host, ssh_port, ssh_user, ssh_password,["ls -l"])
	    ...
	    exec_ssh = self.ssh_execute(ssh_host, ssh_port, ssh_user, ssh_password,"ll", True, False)
            starttime = time.time()
		...
		#post_exec = self.ssh_execute(ssh_host, ssh_port, ssh_user, ssh_password, ["ll"])
                try:
                    if exec_ssh <> None:
                        exec_ssh.close()
...
#        filemagic.close()
	...
	output=""
		...
		s="strace ./aa"
		stdin, stdout, stderr  = ssh.exec_command(s, timeout=20)
